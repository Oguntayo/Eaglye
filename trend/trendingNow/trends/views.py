...
def tweeter(request):
    ...
    for trend in trends[0]["trends"]:
        if trend['tweet_volume'] is not None and int(trend['tweet_volume']) > 90000:
            trend_tweets.append(trend['name'])
    for tweety in trend_tweets:
        for tweet in tweepy.Cursor(api.search_tweets, q=tweety, tweet_mode="extended").items(3):
            trentweets.append(tweet.full_text)
    return JsonResponse({"trends": list(trend_tweets), "tweets": list(trentweets)})
